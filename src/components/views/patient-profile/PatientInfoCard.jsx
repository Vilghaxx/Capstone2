"use client";
import { Calendar, Mail, MapPin, Phone, StickyNote, Trash2, } from "lucide-react";
 ...
 */
export function PatientInfoCard({ patient, canManage, onNotes, onDelete, }) {
    return (<Card>
      ...
      <CardContent>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="space-y-1">
            ...
            {patient.address ? (<p className="flex items-start gap-2 text-sm">
                <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground"/>
                <span>{patient.address}</span>
              </p>) : null}
          </div>
          {patient.notes ? (<div className="space-y-1 sm:col-span-2 lg:col-span-1">
              <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
                Notes
              </p>
