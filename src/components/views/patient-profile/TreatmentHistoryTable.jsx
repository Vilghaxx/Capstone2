 ...
 */
export function TreatmentHistoryTable({ treatments, isLoading, }) {
    const sortedTreatments = useMemo(() => [...treatments].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()), [treatments]);
    if (isLoading) {
        return <LoadingSpinner size="sm"/>;
    }
    if (sortedTreatments.length === 0) {
        return (<p className="rounded-md border border-dashed px-4 py-6 text-center text-sm text-muted-foreground">
    ...
    return (<div className="overflow-x-auto">
      <table className="w-full text-xs xs:text-sm">
        ...
        <tbody>
          {sortedTreatments.map((treatment) => (<tr key={treatment.id} className="border-b last:border-0 hover:bg-muted/40">
