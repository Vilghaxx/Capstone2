...
export default function AppointmentsView() {
    ...
    const openDialog = (presetArg) => {
        setPreset(presetArg !== null && presetArg !== void 0 ? presetArg : {});
        setDialogOpen(true);
    };
    const rolePill = isDentist
        ...
        : isCashier
            ...
            : null;
    return (<div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 lg:py-8 2xl:max-w-[1400px]">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center gap-2">
            ...
            {rolePill ? (<span className={`rounded-full px-2 py-0.5 text-xs font-medium ${rolePill.className}`} aria-label={`Signed in as ${rolePill.label.toLowerCase()}`}>
                {rolePill.label}
              </span>) : null}
          </div>
      ...
      <NewAppointmentDialog open={dialogOpen} onOpenChange={setDialogOpen} preset={preset}/>
    </div>);
}
