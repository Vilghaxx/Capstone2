...
import { CalendarPlus, CheckCircle2, ClipboardList, Info, Loader2, } from "lucide-react";
...
import { APPOINTMENT_TYPES, APPOINTMENT_TYPE_META, APPOINTMENT_TIME_SLOTS, } from "@/lib/format";
...
export default function BookAppointmentView() {
    ...
    const { register, handleSubmit, setValue, control, reset, formState: { errors, isSubmitting }, } = useForm({
        resolver: zodResolver(appointmentFormSchema),
        defaultValues: {
            patientId: (_a = user === null || user === void 0 ? void 0 : user.patientRef) !== null && _a !== void 0 ? _a : "",
            date: "",
            time: "",
            type: "",
            notes: "",
    ...
    if (submitted) {
        return (<div className="mx-auto w-full max-w-[90vw] sm:max-w-2xl px-4 py-10">
        <Card className="border-blue-200 bg-blue-50/60 dark:border-blue-900/50 dark:bg-blue-950/30">
          <CardContent className="flex flex-col items-center gap-4 px-6 py-12 text-center">
            ...
            <div className="mt-4 flex flex-col gap-2 sm:flex-row">
              <Button type="button" variant="outline" onClick={handleBookAnother}>
                <CalendarPlus className="h-4 w-4" aria-hidden="true"/>
                Book Another
              </Button>
              <Button type="button" onClick={() => navigate("my-appointments")} className="bg-blue-600 text-white hover:bg-blue-700">
    ...
    if (profileLoading) {
        return (<div className="mx-auto w-full max-w-[90vw] sm:max-w-md px-4 py-8">
        <Skeleton className="h-8 w-48 mb-6"/>
        <Skeleton className="h-72 w-full rounded-xl"/>
      </div>);
    }
    if ((profileData === null || profileData === void 0 ? void 0 : profileData.patient) && !profileData.patient.phone) {
        return (<div className="mx-auto w-full max-w-[90vw] sm:max-w-2xl px-4 py-10">
          ...
          <CardContent className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <Select value={typeValue} onValueChange={(v) => setValue("type", v, { shouldValidate: true })}>
                <SelectTrigger id="appt-type" className="w-full" aria-invalid={!!errors.type}>
            ...
            return (<SelectItem key={v} value={v}>
              ...
              {errors.type && (<p className="text-xs font-medium text-rose-600" role="alert">
                  {errors.type.message}
                </p>)}
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input id="appt-date" type="date" min={todayStr()} max={oneMonthFromToday()} aria-invalid={!!errors.date} {...register("date")}/>
              {errors.date && (<p className="text-xs font-medium text-rose-600" role="alert">
                  {errors.date.message}
                </p>)}
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={timeValue} onValueChange={(v) => setValue("time", v, { shouldValidate: true })}>
                <SelectTrigger id="appt-time" className="w-full" aria-invalid={!!errors.time}>
              ...
              {errors.time && (<p className="text-xs font-medium text-rose-600" role="alert">
                  {errors.time.message}
                </p>)}
            </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea id="appt-notes" placeholder="Anything we should know?" rows={4} aria-invalid={!!errors.notes} {...register("notes")}/>
