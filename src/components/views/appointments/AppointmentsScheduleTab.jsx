 ...
 */
export function AppointmentsScheduleTab({ onNew, }) {
    ...
    const { data: appointmentsData, isLoading, isError, } = useAppointments({ date: selectedDateYearMonthDay });
    ...
    const { appointmentsByTimeSlot, offGridAppointments } = useMemo(() => {
        ...
        for (const appointment of appointmentsData !== null && appointmentsData !== void 0 ? appointmentsData : []) {
            if (!VISIBLE_SCHEDULE_STATUSES.has(appointment.status))
                continue;
            const time = appointment.time || "";
            if (slotSet.has(time)) {
                const bucket = (_a = slotMap.get(time)) !== null && _a !== void 0 ? _a : [];
    ...
    return (<div className="flex flex-col gap-4 lg:flex-row lg:items-start">
      ...
      <div className="flex-1">
        {isLoading ? (<LoadingSpinner text="Loading schedule…"/>) : isError ? (<EmptyState title="Could not load schedule" message="Something went wrong while fetching appointments. Please try again later."/>) : (<div className="flex flex-col gap-2">
            {APPOINTMENT_TIME_SLOTS.map((time) => {
                var _a;
                const items = (_a = appointmentsByTimeSlot.get(time)) !== null && _a !== void 0 ? _a : [];
                return (<div key={time} className="flex gap-3 rounded-lg border border-border bg-card/30 p-2 xs:p-3 lg:p-4">
                  ...
                  <div className="flex flex-1 flex-col gap-2">
                    {items.length === 0 ? (<button type="button" onClick={() => onNew({ date: selectedDateYearMonthDay, time })} className="flex items-center gap-1.5 self-start rounded-md px-2 py-1 text-sm text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground">
                        <Plus className="h-4 w-4"/> Available
                      </button>) : (items.map((appointment) => {
                        var _a;
                        return (<div key={appointment.id} className="flex items-center justify-between gap-2 rounded-md bg-accent/40 px-3 py-2">
                          <div className="flex flex-col">
                            <span className="text-sm font-medium">
                              {(_a = patientNameById.get(appointment.patientId)) !== null && _a !== void 0 ? _a : "Unknown"}
                            </span>
            ...
            {offGridAppointments.length > 0 && (<div className="mt-2 flex flex-col gap-2 rounded-lg border border-dashed border-border p-3">
                <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
                  Other times
                </div>
                {offGridAppointments.map((appointment) => {
                    var _a;
                    return (<div key={appointment.id} className="flex items-center justify-between gap-2 rounded-md bg-accent/20 px-3 py-2">
                    <div className="flex flex-col">
                      <span className="text-sm font-medium">
                        {(_a = patientNameById.get(appointment.patientId)) !== null && _a !== void 0 ? _a : "Unknown"}{" "}
                        · {appointment.time || "—"}
