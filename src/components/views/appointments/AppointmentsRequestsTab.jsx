...
export function AppointmentsRequestsTab() {
    ...
    const sortedAppointments = useMemo(() => {
        ...
        return [...appointmentsList].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime() ||
    ...
    const onDecline = async (appointment) => {
        try {
            await updateAppointment.mutateAsync({
                id: appointment.id,
    ...
    if (isError) {
        return (<EmptyState title="Could not load requests" message="Something went wrong while fetching pending requests. Please try again later."/>);
    }
    if (sortedAppointments.length === 0) {
        return (<EmptyState title="No pending requests" message="All appointment requests have been reviewed."/>);
    }
          ...
          <CardContent className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1 text-sm">
              ...
              <span className="text-muted-foreground">
                {formatYearMonthDayForDisplay(formatAppointmentDateToYearMonthDay(appointment.date))}{" "}
                at {appointment.time || "—"}
              </span>
              {appointment.notes ? (<span className="text-xs italic text-muted-foreground">
                  &ldquo;{appointment.notes}&rdquo;
                </span>) : null}
            </div>
            <div className="mt-1 flex gap-2">
              <MotionButton size="sm" className="flex-1" onClick={() => onApprove(appointment)} disabled={updateAppointment.isPending} whileTap={{ scale: 0.95 }}>
                <Check className="h-4 w-4"/>
                Approve
              </MotionButton>
              <MotionButton size="sm" variant="outline" className="flex-1" onClick={() => onDecline(appointment)} disabled={updateAppointment.isPending} whileTap={{ scale: 0.95 }}>
                <X className="h-4 w-4"/>
                Decline
              </MotionButton>
            </div>
