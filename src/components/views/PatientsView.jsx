...
import { Calendar, ChevronRight, Mail, Phone, Search, UserPlus, Users, } from "lucide-react";
import { toast } from "sonner";
import { useIsStaff } from "@/lib/auth-store";
import { useNav } from "@/lib/nav";
import { useCreatePatient, useDebounce, usePatients, } from "@/hooks";
...
function PatientCard({ patient, index = 0, }) {
    const navigate = useNav((s) => s.navigate);
    return (<motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} whileHover={{ y: -3 }} whileTap={{ scale: 0.98 }} transition={{ duration: 0.2, delay: index * 0.03 }}>
      <Card role="button" tabIndex={0} onClick={() => navigate("patient-profile", { id: patient.id })} onKeyDown={(e) => {
...
function PatientCardSkeleton() {
    return (<Card className="p-3 xs:p-4 lg:p-5">
      <CardContent className="flex items-start gap-4 px-0">
...
function PatientFormFields({ control, }) {
    return (<>
      ...
      <FormField control={control} name="notes" render={({ field }) => (<FormItem>
...
function AddPatientDialog({ open, onOpenChange }) {
      ...
      <DialogContent className="sm:max-w-[520px]">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => onOpenChange(false)} disabled={createPatient.isPending}>
                Cancel
              </Button>
...
export default function PatientsView() {
    ...
    const totalPages = (_b = data === null || data === void 0 ? void 0 : data.totalPages) !== null && _b !== void 0 ? _b : 1;
    const total = (_c = data === null || data === void 0 ? void 0 : data.total) !== null && _c !== void 0 ? _c : 0;
    return (<div className="mx-auto max-w-7xl space-y-6 2xl:max-w-[1400px]">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        {canManage && (<Button onClick={() => setAddOpen(true)}>
      ...
      <div className="relative w-full sm:w-64 lg:w-80">
        ...
        <Input className="pl-9" placeholder="Search by name, email, or phone…" value={searchInput} onChange={onSearchChange} aria-label="Search patients"/>
      </div>

      {/* Body */}
      {isLoading ? (<div className="grid grid-cols-1 gap-4 xs:grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
          {Array.from({ length: 6 }).map((_, i) => (<PatientCardSkeleton key={i}/>))}
        </div>) : isError ? (<EmptyState title="Could not load patients" message="Something went wrong. Please try again later."/>) : patients.length === 0 ? (<EmptyState title={search ? "No matching patients" : "No patients yet"} message={search
                ? `No patients match “${search}”. Try a different search.`
                : "Add your first patient to get started."} icon={<Users className="h-6 w-6"/>} action={canManage ? (<Button onClick={() => setAddOpen(true)}>
                <UserPlus className="h-4 w-4"/>
                Add Patient
              </Button>) : undefined}/>) : (<>
