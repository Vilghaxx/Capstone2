...
import { TOOTH_STATUS_META, TOOTH_STATUSES, } from "@/lib/format";
 ...
 * ```
 */
export function OralCavityChart({ teeth, onSelectTooth, selectedTooth, }) {
    const [hoveredToothNumber, setHoveredToothNumber] = useState(null);
    ...
    return (<div className="space-y-4">
      ...
      <div className="relative mx-auto w-full max-w-[520px]">
        <div className="relative aspect-square w-full overflow-hidden rounded-2xl border border-border bg-gradient-to-b from-rose-50/40 to-background p-2 shadow-inner dark:from-rose-950/10">
          <svg viewBox="0 0 400 400" className="h-full w-full" role="img" aria-label="Interactive dental chart showing 32 teeth color-coded by status">
            {/* Base oral cavity illustration (400 gradient-shaded paths) —
            injected as native SVG markup so CSS classes render correctly */}
            {baseSvg && (<g dangerouslySetInnerHTML={{ __html: baseSvg }}/>)}
            ...
            {/* Teeth overlay */}
            <g>
              {Object.entries(TOOTH_PATHS).map(([toothNumberString, toothPathData]) => {
            var _a, _b;
            const toothNumber = Number(toothNumberString);
            const tooth = teethByToothNumber.get(toothNumber);
            const status = (_a = tooth === null || tooth === void 0 ? void 0 : tooth.status) !== null && _a !== void 0 ? _a : "healthy";
            const statusColors = (_b = TOOTH_STATUS_COLORS[status]) !== null && _b !== void 0 ? _b : { fill: "#1f2937", stroke: "#111827" };
            const isHovered = hoveredToothNumber === toothNumber;
            const isSelected = selectedTooth === toothNumber;
            const statusDisplayInfo = TOOTH_STATUS_META[status];
            return (<g key={toothNumber}>
                      {/* Soft glow / selection ring */}
                      {(isHovered || isSelected) && (<path d={toothPathData} fill="none" stroke={isSelected ? "#0f172a" : statusColors.stroke} strokeWidth={isSelected ? 2.5 : 2} strokeLinejoin="round" opacity={0.9} style={{ filter: "blur(1px)" }}/>)}
                      <path d={toothPathData} fill={statusColors.fill} fillOpacity={isHovered ? 0.95 : 0.85} stroke={statusColors.stroke} strokeWidth={isSelected ? 2.2 : isHovered ? 1.8 : 1} strokeLinejoin="round" className="cursor-pointer transition-all duration-150" style={{
                    filter: isHovered
                        ? "drop-shadow(0 1px 2px rgba(0,0,0,0.35))"
                        : "drop-shadow(0 1px 1px rgba(0,0,0,0.2))",
                }} onClick={() => onSelectTooth(toothNumber)} onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") {
                        e.preventDefault();
                        onSelectTooth(toothNumber);
                    }
                }} onMouseEnter={() => setHoveredToothNumber(toothNumber)} onMouseLeave={() => setHoveredToothNumber(null)} role="button" tabIndex={0} aria-label={`Tooth ${toothNumber}, ${statusDisplayInfo.label}`}>
            ...
            <line x1={200} y1={20} x2={200} y2={380} stroke="#ffffff" strokeWidth={0.5} strokeDasharray="3 3" opacity={0.2}/>
          </svg>

          {/* Hover tooltip */}
          {hoveredToothNumber != null && (<ToothTooltip toothNumber={hoveredToothNumber} toothRecord={teethByToothNumber.get(hoveredToothNumber)}/>)}
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2 rounded-lg border bg-muted/30 px-4 py-3">
        {ALL_TOOTH_STATUSES.map((status) => {
            ...
            return (<div key={status} className="flex items-center gap-2">
              <span className="h-3.5 w-3.5 rounded-full ring-1 ring-inset ring-black/10" style={{ backgroundColor: statusColors.fill }}/>
 ...
 */
function ToothTooltip({ toothNumber, toothRecord }) {
    var _a;
    const status = (_a = toothRecord === null || toothRecord === void 0 ? void 0 : toothRecord.status) !== null && _a !== void 0 ? _a : "healthy";
    const statusDisplayInfo = TOOTH_STATUS_META[status];
    const statusColors = TOOTH_STATUS_COLORS[status];
    return (<div className="pointer-events-none absolute left-1/2 top-2 z-10 -translate-x-1/2 rounded-lg border border-border bg-popover/95 px-3 py-2 text-center shadow-lg backdrop-blur">
      <div className="flex items-center gap-2">
        <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: statusColors.fill }}/>
        <span className="text-sm font-semibold">Tooth #{toothNumber}</span>
      </div>
      <p className="text-xs text-muted-foreground">
        {statusDisplayInfo.label}
      </p>
      {(toothRecord === null || toothRecord === void 0 ? void 0 : toothRecord.lastTreatment) && (<p className="mt-0.5 max-w-[180px] truncate text-[11px] text-muted-foreground/80">
          {toothRecord.lastTreatment}
        </p>)}
      {(toothRecord === null || toothRecord === void 0 ? void 0 : toothRecord.notes) && (<p className="mt-0.5 max-w-[180px] truncate text-[11px] italic text-muted-foreground/60">
          {toothRecord.notes}
        </p>)}
    </div>);
}
