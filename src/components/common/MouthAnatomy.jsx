...
export const LIP_OUTLINE = "M347.42,201.45c0,107.13-43.94,183.74-145.07,183.74c-94.65,0-145.07-76.62-145.07-183.74 c0-89.74,35.14-169.09,96.85-188.74c13.38-4.26,32.47-1.27,48.57-1.27c16.5,0,35.76-3.18,49.11,0.59 C317.7,30.67,347.42,110.7,347.42,201.45z";
/** Inner mouth opening — the hole inside the lips; also used as a clip path. */
export const MOUTH_OPENING = "M332.68,196.83c0,97.39-46.37,163.56-128.25,163.56c-85.09,0-134.47-65.31-134.47-162.71 S113.8,27.45,204.43,30.67C299.84,34.06,332.68,99.44,332.68,196.83z";
/**
...
export function MouthAnatomy() {
    return (<>
      ...
      <path fillRule="evenodd" clipRule="evenodd" d={`${LIP_OUTLINE} ${MOUTH_OPENING}`} fill="url(#lipRim)"/>

      {/* ===== Details clipped to the inner mouth ===== */}
      <g clipPath="url(#mouthClip)">
        {/* Palatal rugae (ridges on the roof of the mouth) — top area only */}
        <g stroke="#7C2E2A" strokeWidth={1.2} opacity={0.25} fill="none" strokeLinecap="round">
        ...
        <path d="M120,118 Q130,72 204,60 Q280,72 290,118" fill="none" stroke="#F4B0A8" strokeWidth={3.5} opacity={0.25} strokeLinecap="round"/>

        {/* Lower gum highlight */}
        <path d="M120,282 Q130,328 204,340 Q280,328 290,282" fill="none" stroke="#F4B0A8" strokeWidth={3.5} opacity={0.25} strokeLinecap="round"/>

        {/* Tongue (floor of mouth) — lower center only */}
        <ellipse cx={204} cy={335} rx={72} ry={28} fill="url(#tongueGrad)" opacity={0.75}/>
        {/* Tongue central groove */}
        <path d="M172,330 Q204,340 236,330" fill="none" stroke="#883838" strokeWidth={1.5} opacity={0.4} strokeLinecap="round"/>

        {/* Frenum attachments (subtle tissue lines) */}
        <path d="M204,108 L204,128" fill="none" stroke="#A84440" strokeWidth={1.2} opacity={0.35}/>
        <path d="M204,292 L204,272" fill="none" stroke="#A84440" strokeWidth={1.2} opacity={0.35}/>

        {/* Subtle inner vignette (depth at edges only) */}
        <ellipse cx={204} cy={200} rx={130} ry={165} fill="url(#innerShadow)"/>
      </g>

      {/* Lip shine (subtle, on the outer lip area only) */}
      <path d="M170,28 Q204,20 238,28" fill="none" stroke="#FFFFFF" strokeWidth={2.5} opacity={0.18} strokeLinecap="round"/>
      <ellipse cx={204} cy={345} rx={48} ry={6} fill="#FFFFFF" opacity={0.1}/>
    </>);
}
