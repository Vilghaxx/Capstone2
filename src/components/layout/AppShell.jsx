...
import { LayoutDashboard, Users, CalendarDays, Receipt, CalendarPlus, CalendarRange, UserCircle, LogOut, Menu, } from "lucide-react";
...
function NavList({ onNavigate }) {
    ...
    return (<nav className="flex flex-col gap-1">
      {items.map((item) => {
            ...
            return (<button key={item.view} onClick={() => {
                    navigate(item.view);
                    onNavigate === null || onNavigate === void 0 ? void 0 : onNavigate();
                }} className={cn("flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium transition-colors", active
...
function UserCard() {
    ...
    if (!user)
        return null;
    return (<div className="flex items-center gap-3 rounded-md border border-border bg-muted/40 p-3">
